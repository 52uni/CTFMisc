...
if __name__ == '__main__':
    ...
    print u'[*] All files have been backed up!'
    while True:
        ...
        if len(diff_file_list) != 0:
            for filepath in diff_file_list:
                ...
                if Special_string not in f:
                    try:
                        print u'[*] Check and kill suspected files uploaded by WebShell: ' + getUnicode(filepath)
                        shutil.move(filepath, os.path.join(Special_path['webshell'], ntpath.basename(filepath) + '.txt'))
                        print u'[+] The newly uploaded file has been deleted!'
                    except Exception as e:
                        print u'[!] Failed to move file, "%s" maybe webshell, please check it.'%getUnicode(filepath)
                    try:
                        f = open(os.path.join(Special_path['log'], 'log.txt'), 'a')
                        f.write('new_file: ' + getUnicode(filepath) + ' 时间: ' + str(time.ctime()) + '\n')
                        f.close()
                    except Exception as e:
                        print u'[-] Record failed : The uploaded file is: ' + getUnicode(e)
        ...
        for filekey in md5_dict:
            if md5_dict[filekey] != FILE_MD5_DICT[filekey]:
                ...
                if Special_string not in f:
                    try:
                        print u'[*] The file is modified : ' + getUnicode(filekey)
                        shutil.move(filekey, os.path.join(Special_path['difffile'], ntpath.basename(filekey) + '.txt'))
                        shutil.copyfile(os.path.join(Special_path['bak'], ntpath.basename(filekey)), filekey)
                        print u'[+] File restored!'
                    except Exception as e:
                        print u'[!] Failed to move file, "%s" maybe WebShell，please check it.'%getUnicode(filekey)
                    try:
                        f = open(os.path.join(Special_path['log'], 'log.txt'), 'a')
                        f.write('difference_file: ' + getUnicode(filekey) + ' 时间: ' + getUnicode(time.ctime()) + '\n')
                        f.close()
                    except Exception as e:
                        print u'[-] Record failed : Modified file is: ' + getUnicode(filekey)
                        pass
        time.sleep(2)